from app import validar_evento_old as validar_evento
...
def test_con_formato_de_vectores_incorrecto():
    event = {"data": [{"x": 0}], "filename": ""}
    assert validar_evento(event) == False


def test_data_no_es_array():
    event = {"data": 0, "filename": ""}
    assert validar_evento(event) == False


def test_filename_no_es_string():
    event = {"data": [], "filename": 1}
    assert validar_evento(event) == False
...
def test_sin_vectores():
    event = {"filename": "nombre", "time_start": "timestamp", "data": [2, 3]}
    assert validar_evento(event) == False
